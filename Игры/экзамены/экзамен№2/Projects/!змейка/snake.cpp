...
enum color{BLACK, DARKBLUE, DARKGREEN, DARKCYAN, DARKRED, DARKPURPLE, DARKYELLOW,
    LOWWHITE, GREY, BLUE, GREEN, CYAN, RED, PURPLE, YELLOW, WHITE};// Перечисление цветов
...
void mainMenu()
{
    ...
    while(true)
    {
        ...
        setColor(DARKGREEN, BLACK);
        gotoXY(38, 48);
        cout <<  "Artem Alekseev 2011" << char(169);
...
void loadLevel(int n)
{
    ...
    {
        strcpy(path, "data/levels/mylevel.txt");
        f = fopen(path, "a+");
    }
    else// Режим карьеры
    {
...
void editor()
{
    ...
    { 
        for(int j = 0; j < widthField; ++j)
        {
            ...
            {
                fclose(f);
                f = fopen(path, "w+");// Перезапись файла
                for(int i = 0; i < heightfield; ++i)// Обнуление массива
                { 
                    for(int j = 0; j < widthField; ++j)
                    {
    ...
    while(true)
    {
        ...
        for(int i = 0; i < e; ++i)
        {
            ...
            else if (all_events[i].Event.MouseEvent.dwButtonState==FROM_LEFT_1ST_BUTTON_PRESSED && c.Y == 10 && c.X >= 86 && c.X <=94)// Сохранить
            {
                ...
                f = fopen("data/levels/mylevel.txt", "w+");
                for(int i = 0; i < heightfield; ++i)
                { 
                    for(int j = 0; j < widthField; ++j)
                    {
...
void game(bool career)
{
    ...
    setlocale(0, "RUS");// Отрисовка вспомогательной информации
    if(career)
    {
        gotoXY(84, 12);
        setColor(RED, BLACK);
    ...
    while(true)// Игровой цикл
    {
        ...
        while(kbhit())// Отлавливание нажатия клавиш
        {
            key = getch();
