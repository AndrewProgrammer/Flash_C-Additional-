...
VOID CALLBACK TimerProcedure(HWND window, UINT message, UINT_PTR idEvent, DWORD interval);
...
VOID CALLBACK TimerProcedure(
    HWND window, // Дескриптор окна, которое собиратеся использовать таймер.
    UINT /*message*/, // Идентификатор сообщения WM_TIMER.
    UINT_PTR /*idEvent*/, // Индентификатор установленного таймера.
    DWORD /*interval*/ // Временной интервал для таймера в миллисекундах.
)
{
    TCHAR caption[100] = {};
                ...
                switch (wParam)
                {
                    ...
                    case VK_RETURN:
                        ...
                        SetTimer(
                            ...
                            TimerProcedure // Указатель на функцию-обработчик прерываний таймера.
                        );
                        break;
                }
            }
